...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zpp2Tracker->HasMintTx(hash))) {
            //zPP2 stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZPP2;
...
bool IsZPP2Type(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZPP2:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZPP2 || type == TransactionRecord::MNReward) {
