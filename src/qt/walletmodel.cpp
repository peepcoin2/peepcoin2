...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered peepcoin2 address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 PP2.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal peepcoin2:URI (peepcoin2:XyZ...?message=example)
            {
...
static void NotifyzPP2Reset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzPP2Reset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzPP2Reset.connect(boost::bind(NotifyzPP2Reset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzPP2Reset.disconnect(boost::bind(NotifyzPP2Reset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zpp2Tracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
